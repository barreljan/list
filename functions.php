...
function __dispHtmlErrorpage__($error,$issue) {
	...
	if ($error == "NO_ITEM") {
		...
		echo "   <br>\n   Group can not be removed, not empty!";
	} if ($error == "SESS_HIJACK") {
		echo "   <br>\n   This is not allowed!!";
		$subject = "Website violation op List";
		$from = "no-reply@somedomain.tld";
		$msg = "There is a session violation detected on the website\n\n";
		...
		$headers[] = "From: Some webserver <{$from}>";
		$headers[] = "Reply-To: No-Reply <{$from}>";
		$headers[] = "X-Mailer: PHP/".phpversion();
		mail ("admin@someaddress.com",$subject,$msg,implode("\r\n", $headers),"-f {$from}");
	} if ($error == "SHORTPASS") {
		echo "   <br>\n   Password too short, minimal of 8 characters please!";
	} if ($error == "NO_SESSION") {
		echo "   <br>\n   Oops, you have to login again, there is no active session here...";
	}
	__dispHtmlfooter__();
}
